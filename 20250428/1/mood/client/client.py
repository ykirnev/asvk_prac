...
class MUDClient(cmd.Cmd):
    """Клиент многопользовательской игры (MUD) для взаимодействия с сервером MOOD."""
    prompt = "MUD> "
    ...
    async def do_addmon(self, arg: str):
        ...
        Формат: <имя> hello <сообщение> hp <здоровье> coords <x> <y>
        """
    ...
    async def do_attack(self, arg: str):
        ...
        Формат: <имя_монстра> [with <оружие>]
        """
        args = shlex.split(arg)
    ...
    async def do_movemonsters(self, arg: str):
        """Включить или выключить режим бродячих монстров."""
        if arg not in ("on", "off"):
            print("Неверные аргументы: ожидается 'on' или 'off'")
            return False
        command = f"movemonsters {arg}"
        print(f"Debug: Sending command: {command}")
        await self.send_command(command)
        return False

    async def do_locale(self, arg: str):
        """Установить локаль для сообщений сервера."""
        args = shlex.split(arg)
        print(f"Debug: locale args: {args}")
        if len(args) != 1:
            print("Неверные аргументы: ожидается имя локали")
            return False
        command = f"locale {args[0]}"
        print(f"Debug: Sending command: {command}")
        await self.send_command(command)
        return False
