                ...
                if cmd[0] == "move":
                    ...
                    if pos not in game_state.monsters or game_state.monsters[pos][0] != monster_name:
                        ...
                        try:
                            ...
                            print(f"Debug: Sending attack response: {message}")
                            writer.write(message.encode() + b"\n")
                        ...
                        if hp == 0:
                            ...
                            print(f"Debug: Sending death response: {message}")
                            writer.write(message.encode() + b"\n")
                            ...
                            for u, w in game_state.clients.items():
                                ...
                                print(f"Debug: Sending broadcast kill: {message}")
                                w.write(message.encode() + b"\n")
                        else:
                            message = translations.ngettext(
                                "{name} now has {hp} hp",
                            ...
                            print(f"Debug: Sending hp response: {message}")
                            writer.write(message.encode() + b"\n")
                            for u, w in game_state.clients.items():
                                w_translations = load_translations(getattr(w, 'locale', 'en_US'))
                                try:
                                ...
                                print(f"Debug: Sending broadcast attack: {message}")
                                w.write(message.encode() + b"\n")
                        await writer.drain()
                elif cmd[0] == "sayall":
                    if len(cmd) < 2:
...
def start_server(no_monsters: bool = False):
    """Функция для запуска сервера в отдельном процессе."""
    asyncio.run(run_server(no_monsters=no_monsters))
...
def main():
    ...
    start_server(no_monsters=args.no_monsters)
